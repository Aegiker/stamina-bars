...
void Play_DrawMirrorScanline(PlayState* this, GraphicsContext* gfxCtx, u32 scanline, u32 drawPosX, u32 drawPosY, f32 scaleX, f32 scaleY, u8 mirrorX, Gfx** gfxP, void* texPtr) {
    ...
    gDPLoadTextureTile(gfx++, (u32)texPtr + texOffset, G_IM_FMT_RGBA, G_IM_SIZ_16b, SCREEN_WIDTH, SCREEN_HEIGHT, uls, ult, lrs, lrt, 0,
                           G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
                           G_TX_NOLOD);

    gSPTextureRectangle(gfx++, drawPosX << 2, drawPosY << 2, (s32)(drawPosX + width) << 2, 
                        (s32)((drawPosY + scanlineHeight) * scaleY) << 2, G_TX_RENDERTILE, s, 0, sx * (u32)((1 << 10) / scaleX), (u32)((1 << 10) / scaleY));
...
void Play_DrawMirrorMode(PlayState* this, GraphicsContext* gfxCtx, Gfx** gfxP, void* texPtr) {
    ...
    gDPSetTileCustom(gfx++, G_IM_FMT_RGBA, G_IM_SIZ_16b, 0, 0, SCREEN_WIDTH, SCANLINE_DEFAULT, 0, G_TX_NOMIRROR | G_TX_CLAMP,
...
void Play_Draw(PlayState* this) {
    ...
    if (R_ENABLE_MIRROR) {
        if (this->pauseCtx.state < 2) {
            ...
            mirrorStatus = MIRROR_STATUS_PAUSE_BG;
        }
    } else {
        mirrorStatus = MIRROR_STATUS_OFF;
    }
    ...
    if (!DEBUG_FEATURES || (R_HREG_MODE != HREG_MODE_PLAY) || R_PLAY_RUN_DRAW) {
        ...
        if (R_PAUSE_BG_PRERENDER_STATE == PAUSE_BG_PRERENDER_READY) {
            ...
            if (mirrorStatus == MIRROR_STATUS_PAUSE_BG) {
                Play_DrawMirrorMode(this, gfxCtx, &gfxP, gfxCtx->curFrameBuffer);
            }
            POLY_OPA_DISP = gfxP;
